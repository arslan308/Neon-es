...
var SearchBar = class {
  ...
  _handleDocumentClick(event) {
    const isInsideSearchBar = this.element.contains(event.target);
    const isToggleSearch = event.target.closest('[data-action="toggle-search"]') !== null;

    if (!isInsideSearchBar && !isToggleSearch) {
      // Remove classes and update isSearchOpen
      this.element.classList.remove("is-visible");
